...
def _build_system_prompt(brand_name: str, brand_tagline: str) -> str:
    return f"""You are an expert copywriter and content manager for {brand_name} — "{brand_tagline}".

Your task: The user will provide a brief prompt or summary of an event, product, or thought.
You must expand this into a short, engaging, professional 2-3 paragraph blog post written in {brand_name}'s voice.
...
async def generate_blog_post(prompt: str) -> str:
    ...
    system_prompt = _build_system_prompt(settings.brand_name, settings.brand_tagline)
