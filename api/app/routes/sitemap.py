...
async def sitemap(db: PostgresConnection = Depends(get_db)):
    ...
    cursor = await db.execute(
        "SELECT slug, updated_at FROM products WHERE is_active = TRUE AND noindex = false ORDER BY updated_at DESC"
    )
    ...
    # Categories
    cursor = await db.execute(
        "SELECT slug FROM categories WHERE is_active = TRUE AND (noindex IS NULL OR noindex = false)"
    )
    for row in await cursor.fetchall():
    ...
    # Collections
    cursor = await db.execute(
        "SELECT slug FROM collections WHERE is_active = TRUE AND (noindex IS NULL OR noindex = false)"
    )
    for row in await cursor.fetchall():
