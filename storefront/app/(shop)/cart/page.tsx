  ...
  if (items.length === 0) {
    return (
      <div className="max-w-2xl mx-auto px-4 py-16 text-center">
        ...
        <Link href="/" className="inline-block bg-brand text-white px-6 py-3 rounded-lg font-medium hover:bg-brand/90">
          Continue Shopping
        </Link>
      </div>
    );
    ...
    <div className="max-w-2xl mx-auto px-4 py-8">
      ...
      {shippingCost > 0 && (
        <div className="mb-6 p-4 bg-gray-50 rounded-xl">
          <p className="text-sm text-gray-600">{formatCents(remainingForFree)} away from free shipping</p>
        ...
        {items.map((item) => (
          <div key={item.variantId} className="flex gap-4 p-4 bg-white border border-gray-100 rounded-xl">
            ...
            <div className="flex flex-col items-end justify-between">
              <button onClick={() => removeItem(item.variantId)} className="text-gray-400 hover:text-red-500 transition-colors" aria-label="Remove">
                <Trash2 size={16} />
              </button>
              <div className="flex items-center gap-2 border border-gray-200 rounded-lg">
                <button onClick={() => updateQuantity(item.variantId, item.quantity - 1)} className="w-8 h-8 flex items-center justify-center text-gray-600 hover:text-brand">
                  <Minus size={14} />
                </button>
      ...
      <Link href="/checkout" className="mt-6 block w-full text-center bg-brand text-white py-4 rounded-xl font-bold hover:bg-brand/90 active:scale-[0.98] transition-all">
