  ...
  async function handleSave() {
    ...
    try {
      await api.patch('/api/admin/social/persona', {
        ...
        brand_owner_pronouns: persona.brand_owner_pronouns,
        cultural_identity: persona.cultural_identity,
      });
  ...
  return (
    <div className="max-w-2xl space-y-6">
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-6 space-y-6">
        ...
        {/* Identity fields — used for image prompt representation */}
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label htmlFor="persona-pronouns" className="text-sm font-medium text-gray-700 block mb-1">Brand Owner Pronouns</label>
            <select
              id="persona-pronouns"
              value={persona.brand_owner_pronouns}
              onChange={(e) => handleChange('brand_owner_pronouns', e.target.value)}
            ...
            <p className="mt-1 text-xs text-gray-400">Used in AI image prompts to show the right person.</p>
          </div>
          <div>
            <label htmlFor="persona-cultural" className="text-sm font-medium text-gray-700 block mb-1">Cultural / Nation Identity</label>
            <input
              id="persona-cultural"
              type="text"
              value={persona.cultural_identity}
              onChange={(e) => handleChange('cultural_identity', e.target.value)}
              className="w-full px-4 py-2.5 rounded-lg border border-gray-200 focus:border-brand outline-none text-sm"
              placeholder="e.g. Mohawk, Haudenosaunee Confederacy"
            />
            <p className="mt-1 text-xs text-gray-400">Injected into image prompts for authentic representation.</p>
          </div>
        </div>
